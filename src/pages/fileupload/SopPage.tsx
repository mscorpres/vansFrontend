...
import { findFolderNameById, FolderDataType } from "@/shared/sop/folderData";
import {
  createFolder,
  deleteFolder,
  getFileAsync,
...
export default function SopPage() {
  ...
  const handleAddFolder = () => {
    if (!folderName) {
      ...
      dispatch(createFolder({ id: target, name: folderName })).then(
        (res: any) => {
    ...
    return (
      <div key={folder.folderId}>
        ...
        >
          <ListItemIcon>
            {isOpen ? (
              <FaFolderOpen className="h-[18px] w-[18px]" />
            ) : (
              <FaFolder className="h-[18px] w-[18px]" />
            )}
          </ListItemIcon>
    ...
    <>
      ...
      >
        ...
        <DialogContent sx={{ width: "600px" }}>
          ...
          >
            ...
            Do you Want to delete <b>
              {findFolderNameById(folders, target)}
            </b>{" "}
            folder? This action will permanently delete the selected folder and
            all of its contents, including any subfolders and files. This
            operation cannot be undone. Do you still want to proceed?
          </DialogContentText>
          <Typography sx={{ my: 2 }}>
            To confirm, type{" "}
            <b>"delete/{findFolderNameById(folders, target)}"</b> in the box
            below
          </Typography>
          <TextField
            size="small"
        ...
        <DialogActions>
          ...
          <Button
            disabled={
              deleteInput !== `delete/${findFolderNameById(folders, target)}` ||
              deleteFolderLoading
            }
            variant="contained"
            color="error"
      ...
      >
        <div className="absolute top-0 left-0 right-0">
          {createFolderLoading && <LinearProgress />}
        </div>
      ...
      >
        <div className="absolute top-0 left-0 right-0">
          {uploadFileLoading && <LinearProgress />}
        </div>
        <div className="flex flex-col px-[20px] h-[70px] justify-center gap-[5px]">
          <Typography
            variant="h3"
