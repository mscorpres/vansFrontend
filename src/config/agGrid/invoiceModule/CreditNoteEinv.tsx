...
import { createCreditEinvoice, createEwayBill, fetchDataForEwayBill, fetchDataForInvoice, fetchNoteData, generateEInvoice } from "@/features/salesmodule/salesInvoiceSlice";
...
export default function CreateEwayBill() {
 ...
 const isCnDn =
  ...
  if (isCnDn) {
    dispatch(fetchNoteData({ note_no: shipmentId })).then((res: any) => {
      if (res.payload.success) {
        const materials = res.payload.items || []; 
        console.log("materials", materials);
        setRowData(materials);
        const data = res.payload?.data;
        console.log("einvoice cred dataa", data);
        form.setValue("header.documentNo", data?.documentNo);
        form.setValue("header.other_ref", data?.so_note_other_ref); // Use 'so_note_other_ref'
        isCrNote
          ? form.setValue("header.creditNo", data?.so_note_id)
          : form.setValue("header.debitNo", data?.so_note_id); // Use 'so_note_id' for both
        setOrderId(data?.so_note_id);
        ...
        form.setValue("billFrom.pincode", data?.bill_from?.pincode);
        form.setValue("billFrom.pan", data?.bill_from?.pan);
        form.setValue("billFrom.email", data?.bill_from?.email || ""); // Handle missing email
        form.setValue("billFrom.phone", data?.bill_from?.phone || ""); // Handle missing phone
        form.setValue("billFrom.addressLine1", data?.bill_from?.address1);
        form.setValue("billFrom.addressLine2", data?.bill_from?.address2);
        ...
        form.setValue("billTo.pincode", data?.bill_to?.pincode);
        form.setValue("billTo.pan", data?.bill_to?.pan);
        form.setValue("billTo.email", data?.bill_to?.email || "");
        form.setValue("billTo.phone", data?.bill_to?.phone || "");
        form.setValue("billTo.addressLine1", data?.bill_to?.address1);
        form.setValue("billTo.addressLine2", data?.bill_to?.address2);
 ...
 const onSubmit = (payload: any) => {
  ...
  if (isCnDn) {
    if (isCrNote) {
      dispatch(createCreditEinvoice(modifiedPayload)).then((response) => {
        console.log("response", response);
        if (response.payload.success) {
          toast({
            title: response?.payload?.message || "Data Submitted Successfully",
